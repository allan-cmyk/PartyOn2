...
export default function BoatPartiesProductsPage() {
  ...
  // Fetch products tagged with boat party related tags
  const { products, loading, error } = useTaggedProducts({
    tags: ['boat-party', 'lake-party', 'yacht', 'party-package'],
  });
  ...
  const curatedCollections = useMemo(() => {
    const lakeDayProducts = products.filter((p: ShopifyProduct) =>
      p.tags.some((tag: string) => tag.toLowerCase().includes('lake') || tag.toLowerCase().includes('boat')) &&
      !p.tags.some((tag: string) => tag.toLowerCase().includes('yacht') || tag.toLowerCase().includes('luxury'))
    );

    const yachtClubProducts = products.filter((p: ShopifyProduct) =>
      p.tags.some((tag: string) => tag.toLowerCase().includes('yacht') || tag.toLowerCase().includes('luxury') || tag.toLowerCase().includes('premium'))
    );

...
// ──────────────────────────────────────────────
// Order Cancelled Notification (CoreLinq only)
// ──────────────────────────────────────────────
...
/**
 * Build the order.cancelled payload. Exported for cancelOrder and its tests.
 */
export function buildOrderCancelledPayload(
  ...
  return {
    ...
    email: order.customerEmail,
    phone: order.customerPhone || order.deliveryPhone || '',
    customerName: order.customerName,
    refunded: !!refund,
...
/**
 * Tell the CRM an order was cancelled so it texts the customer (order #527:
 * a cancel that only emailed left the customer's SMS thread ending at
 * "your order is confirmed" while the delivery never came).
 *
 * CoreLinq only — no GHL leg; GHL was decommissioned before this event
 * existed. Fire-and-forget via postToCoreLinq: logs + alerts on failure,
 * never throws.
 */

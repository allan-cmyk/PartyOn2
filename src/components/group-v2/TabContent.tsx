...
export default function TabContent({
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Status banners */}
      {tab.status === 'LOCKED' && (
        <div className="bg-yellow-50 border-2 border-yellow-300 rounded-xl p-4 text-base text-yellow-800 flex items-center gap-3">
          <svg className="w-6 h-6 shrink-0" fill="currentColor" viewBox="0 0 20 20">
            <path fillRule="evenodd" d="M5 9V7a5 5 0 0110 0v2a2 2 0 012 2v5a2 2 0 01-2 2H5a2 2 0 01-2-2v-5a2 2 0 012-2zm8-2v2H7V7a3 3 0 016 0z" clipRule="evenodd" />
          </svg>
          <span className="font-medium">This tab is locked by the host. You can still checkout your existing items.</span>
        </div>
      )}

      {isPastDeadline && tab.status === 'OPEN' && (
        <div className="bg-amber-50 border-2 border-amber-300 rounded-xl p-4 text-base text-amber-800 flex items-center gap-3">
          <svg className="w-6 h-6 shrink-0" fill="currentColor" viewBox="0 0 20 20">
            <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm1-12a1 1 0 10-2 0v4a1 1 0 00.293.707l2.828 2.829a1 1 0 101.415-1.415L11 9.586V6z" clipRule="evenodd" />
          </svg>
          <span className="font-medium">The ordering deadline has passed. You can still checkout your existing items.</span>
        </div>
      )}

      {tab.status === 'CANCELLED' && (
        <div className="bg-red-50 border-2 border-red-300 rounded-xl p-4 text-base text-red-800 flex items-center gap-3">
          <svg className="w-6 h-6 shrink-0" fill="currentColor" viewBox="0 0 20 20">
            <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM8.707 7.293a1 1 0 00-1.414 1.414L8.586 10l-1.293 1.293a1 1 0 101.414 1.414L10 11.414l1.293 1.293a1 1 0 001.414-1.414L11.414 10l1.293-1.293a1 1 0 00-1.414-1.414L10 8.586 8.707 7.293z" clipRule="evenodd" />
          </svg>
          <span className="font-medium">This delivery was cancelled by the host.</span>
        </div>
      )}

      {tab.status === 'FULFILLED' && (
        <div className="bg-green-50 border-2 border-green-300 rounded-xl p-4 text-base text-green-800 flex items-center gap-3">
          <svg className="w-6 h-6 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
          </svg>
          <span className="font-medium">This delivery is complete!</span>
        </div>
      )}

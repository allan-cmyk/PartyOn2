...
/**
 * @fileoverview Add topic cluster links to existing blog posts
 * @module scripts/add-cluster-links
 *
 * This script updates existing blog posts to include:
 * 1. A link back to their pillar page
 * 2. Related article links to sibling cluster pages
 *
 * Usage: npx tsx scripts/add-cluster-links.ts
 */

import fs from 'fs';
import path from 'path';

// Topic cluster definitions (mirrored from src/lib/topic-clusters.ts)
...
function findPillarForSlug(slug: string): TopicCluster | undefined {
  return TOPIC_CLUSTERS.find(cluster => cluster.clusterSlugs.includes(slug));
}

function getRelatedSlugs(slug: string, cluster: TopicCluster, limit = 3): string[] {
  return cluster.clusterSlugs
    .filter(s => s !== slug)
    .slice(0, limit);
}
...
function slugToTitle(slug: string): string {
  return slug
    ...
    .map(word => {
      ...
      if (word === 'at') return 'at';
      if (word === 'ut') return 'UT';
      if (word === 'bbq') return 'BBQ';
      if (word === 'diy') return 'DIY';
      if (word === 'vip') return 'VIP';
      if (word === 'av') return 'A/V';
      if (word === 'byob') return 'BYOB';
      return word.charAt(0).toUpperCase() + word.slice(1);
    })
    .join(' ')
    .replace(/ 's/g, "'s"); // Fix possessives
}

function generateClusterSection(slug: string): string {
  const cluster = findPillarForSlug(slug);
  if (!cluster) return '';

  const relatedSlugs = getRelatedSlugs(slug, cluster, 3);
  const relatedLinks = relatedSlugs.map(s => `- [${slugToTitle(s)}](/blog/${s})`).join('\n');

  return `

---

## More ${cluster.category} Resources
...
function hasClusterLinks(content: string): boolean {
  return content.includes('This article is part of our comprehensive') ||
         content.includes('This guide is part of our Austin Event Planning series');
}

function addClusterLinksToPost(filepath: string): boolean {
  const content = fs.readFileSync(filepath, 'utf-8');
  const filename = path.basename(filepath, '.mdx');

  // Skip if already has cluster links
  if (hasClusterLinks(content)) {
    console.log(`  ⏭️  Skipped (already has links): ${filename}`);
    return false;
  }

  // Skip pillar pages
  const isPillar = TOPIC_CLUSTERS.some(c => c.pillarSlug === filename);
  if (isPillar) {
    console.log(`  ⏭️  Skipped (pillar page): ${filename}`);
    return false;
  }

  // Find cluster for this post
  const cluster = findPillarForSlug(filename);
  if (!cluster) {
    console.log(`  ⏭️  Skipped (no cluster match): ${filename}`);
    return false;
  }

  // Generate cluster section
  const clusterSection = generateClusterSection(filename);

  // Find where to insert (before the schema.org script if present, otherwise at end)
  let newContent: string;
  const schemaMatch = content.match(/<script type="application\/ld\+json"/);

  if (schemaMatch && schemaMatch.index) {
    // Insert before schema script
    newContent = content.slice(0, schemaMatch.index) + clusterSection + '\n' + content.slice(schemaMatch.index);
  } else {
    // Append to end
    newContent = content + clusterSection;
  }
...
async function main() {
  console.log('\n' + '='.repeat(60));
  console.log('  Adding Topic Cluster Links to Blog Posts');

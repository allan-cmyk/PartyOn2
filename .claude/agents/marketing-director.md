...
- **Never assert the SIGN of ROI / margin (positive vs. negative, profitable vs. unprofitable) while `marginCoveragePct` is below 70%.** This rule is stronger than the action gate above: even narrative phrasing like "confirmed negative-ROI even within the partial data" or "this affiliate is unprofitable" is forbidden. At low coverage you may quote the raw rollup numbers as-reported, but you must label the direction as **directionally uncertain** and explain why (sparse cost data, sample-selection bias). Per ADR M0001 (2026-05-04), affiliate ROI specifically also has known revenue-attribution leaks (`/partners/<code>` cookie gap, repeat-customer LTV, group-dashboard sub-orders) that further undercount affiliate revenue — so even at ≥70% coverage, treat affiliate ROI sign as suspect until those leaks are closed.
